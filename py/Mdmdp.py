...
lb3 = Label(root, text='compressed-x-grps(更改组别参数如protein non-protein)')
lb3.pack()
inp3 = Entry(root)
inp3.insert(0, "system")
...
def function():
    ...
    ref_get = str(inp9.get())
    #作一步计算
    nsteps_get = float(SimulationTime_get) * 1000 / float(dt_get)
    nsteps_get = str(nsteps_get)
    nsteps_get = nsteps_get.replace(".0", "")
    #加载模板
    template = "./model/Mdmdp.docx"

    document = MailMerge(template)
    print("Fields included in {}: {}".format(template, document.get_merge_fields()))
    document.merge(
        nsteps=nsteps_get,              #这些是ns数，如500000*0.002是1ns
        dt=dt_get,                  #这些是ns数，如500000*0.002是1ns
        compressed_x_grps=compressed_get,   #更改组别参数（如protein non-protein）
    ...
    document.write('Md-MdpOut.docx')
    path = "Md-MdpOut.docx"
    doc = Document(path)
    print("----------生成模板中-----------")
    #批量写入到mdp中
    with open("Md-MdpOut.mdp", "w") as file:
